...
public class AstralJumpState : AstralBaseState{
    private readonly int JumpHash = Animator.StringToHash("Jump");

    private const float CrossFadeDuration = 0.1f;
    private float force;

    public AstralJumpState(AstralStateMachine stateMachine) : base(stateMachine) {}
    ...
    public override void Tick(float deltatime) {
        ...
        if (stateMachine.AstralInputReader.IsAttacking && stateMachine.AstralInputReader.MovementValue.y > 0f) {
            ...
            stateMachine.SwitchState(new AstralAirAttackState(stateMachine, 0));
            return;
        }
        if (stateMachine.AstralInputReader.IsAttacking) {

            stateMachine.Rigidbody.velocity = Vector3.zero;
            stateMachine.SwitchState(new AstralAirSideAttackState(stateMachine, 0));
            return;
        }
        #endregion

...
public class EliteMeleePatrolState : EliteMeleeBaseState{

    public EliteMeleePatrolState(EliteMeleeStateMachine stateMachine) : base(stateMachine) {}
    ...
    public override void Enter() {
        ...
        if(randomValue > 0f && !IsMovingRight){
            ...
            RandomPos = stateMachine.RanPatrolPos[0];  
            IsMovingRight = true;
        }
        else if(randomValue < 0f && IsMovingRight){

            RandomPos = stateMachine.RanPatrolPos[1];
            IsMovingRight = false;  
        }
